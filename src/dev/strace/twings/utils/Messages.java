...
public class Messages extends ConfigManager {
    ...
    public String getAlreadyequipped() {
        return MyColors.format(alreadyequipped.replace("%prefix%", prefix));
    }
    public String getMaxtwings() {
        return MyColors.format(maxtwings.replace("%prefix%", prefix).replace("%max%", Main.getInstance().getMaxEquippedTwings()+""));
    }
}
